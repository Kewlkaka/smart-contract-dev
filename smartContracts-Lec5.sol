...
// 1. Burnable Token
//inherits ERC20Burnable, to utilize its Burn method.
...
// 4. Pausable Token
//Pausable token, is a token where owner has authority to pause/unpause transactions
//done via shifting of bool values from true and false
...
// 5. Taxed (Fee) Token
//beneficiary fee charges implementation
contract TaxedToken is ERC20, Ownable {
    ...
    //a custom transfer, modified to our liking
    //in this case certain users are not taxed
...
// 6. Reflection Token
//all holders of a token are given a percentage of the tax when a user sends/transacts tokens
contract ReflectionToken is ERC20 {
    ...
    //custom logic as to how one would want reflections to be handled, for instance per token
    //like giving a holder of more tokens; a greater percentage of the reflection
    constructor(string memory name, string memory symbol) ERC20(name, symbol) {
        _mint(msg.sender, 1000000 * 10 ** decimals());
    ...
    function _customTransfer(address sender, address recipient, uint256 amount) internal {
        //make sure none are zero addresses
        require(sender != address(0), "ERC20: transfer from zero address");
        require(recipient != address(0), "ERC20: transfer to zero address");
...
// in reflection the reward is based on tokens
// in dividend, it can be any currency
...
contract DividendPayingToken is ERC20, Ownable {
    ...
    function distributeDividends(uint256 amount) public onlyOwner {
        ...
        if (amount > 0) {
            //.add is now obsolete, we use + directly
            //share calculated
    ...
    //acts as a claim, where user can claim their dividend.
...
// 8. Rebasing Token
//total supply is rebased to maintain the price
//if i say my token is worth $1 and price is falling
//we can use rebase to cut down supply, and vice versa; to mint
...
// Wrapped token acts as a wrapper on other blockchains.
// e.g WETH ~ ETH, where WETH's value would be maintained on the basis of ETH via some algorithm
...
contract Stablecoin is ERC20, Ownable {
    //stable coin is a token that is pegged by something
    // for instance USDT, Gold, and hence priced accordingly
    // this means that the stable coin is a token in some exchange
    // its value's rise and drop is via collateral balance
